...
function SwRegisterPlugin(options = {}) {
    let filePath = path.resolve(
        cwd,
        options.filePath || "./src/sw-register.js"
    );
...
SwRegisterPlugin.prototype.apply = function(compiler) {
    let me = this;
    let swRegisterEntryFilePath = path.resolve(
        __dirname,
        "templates",
        "sw-register-entry.js.tpl"
    );
    ...
    let emitCallback = (compilation, callback) => {
        ...
        if (me.entries.length === 0) {
            ...
            me.entries.forEach(entryConfig => {
                ...
                let swName =
                    entryConfig.serviceWorker.swName ||
                    `/${entryName}/service-worker.js`;
                let swRegisterName =
                    entryConfig.serviceWorker.swRegisterName ||
                    `${entryName}/${me.fileName}`;
                let scope = entryConfig.serviceWorker.scope;

                if (!scope) {
                    scope =
                        entryConfig.urlReg.toString() === "/^\\//"
                            ? "/"
                            : `/${entryName}/`;
                }
                ...
                let entryContent = con
                    .replace(
                        /\.register\(([^\)]+)\)/,
                        `.register($1, {scope: '${scope}'})`
                    )
        ...
        Object.keys(compilation.assets).forEach(asset => {
            ...
            if (
                !isIn(asset, me.excludes) &&
                (/\.html$/.test(asset) || isIn(asset, me.includes))
            ) {
                let htmlContent = compilation.assets[asset].source().toString();
                let swRegisterEntryFileTpl = fs.readFileSync(
                    swRegisterEntryFilePath,
                    "utf-8"
                );
                ...
                if (me.entries.length !== 0) {
                    ...
                    swRegisterEntryFileContent = etpl.compile(
                        swRegisterEntryFileTpl
                    )({
                ...
                htmlContent = htmlContent.replace(
                    /<\/body>/,
                    `${swRegisterEntryFileContent}</body>`
                );
